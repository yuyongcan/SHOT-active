...
folder = '/data/yongcan.yu/VisDA-C/'
target_folder='./data/VISDA-C'
...
for d in range(len(domains)):
	...
	if os.path.isdir(os.path.join(folder, dom)):
		...
		classes.sort()
		# print(classes)
		f = open(os.path.join(target_folder,dom_new + "_list.txt"), "w")
		for c in range(len(classes)):
			cla = classes[c]
			cla_new = cla.replace(" ","_")
			print(cla, cla_new)
			os.rename(os.path.join(folder, dom_new, cla), os.path.join(folder, dom_new, cla_new))
			if not os.path.isdir(os.path.join(folder, dom_new, cla_new)):
				continue
